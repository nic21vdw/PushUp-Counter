...
test('a rep makes a noise unless you ask it not to', () => {
  assert.equal(parse('').sound, 'sayings', 'a different line each rep, so it stays funny');
  ...
  assert.equal(parse('sound=not a name!').sound, 'sayings', 'but that is not a file name');
});
...
test('the sound can be chosen by name', () => {
  ...
  assert.equal(parse('sound=1').sound, 'sayings', 'the old on-switch still means the default');
  assert.equal(parse('sound=').sound, 'sayings', 'a bare ?sound is not a request for silence');
  assert.equal(parse('sound=shuffle').sound, 'shuffle', 'the noises are still one pick away');
  assert.equal(
